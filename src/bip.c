...
// FreeRTOS Modification - Fix implicit declarations of memcpy and memset.
#include <string.h>
#include "bacdcode.h"
#include "bacint.h"
#include "bip.h"
...
// FreeRTOS Modification - Change type from int to Socket_t and replace -1 with NULL.
static Socket_t BIP_Socket = NULL;
 ...
 */
// FreeRTOS Modification - Change sock_fd type from int to Socket_t.
void bip_set_socket(
    Socket_t sock_fd)
{
    BIP_Socket = sock_fd;
 ...
 */
// FreeRTOS Modification - Change return type from int to Socket_t.
Socket_t bip_socket(
    void)
{
    return BIP_Socket;
...
bool bip_valid(
    void)
{
    // FreeRTOS Modification - Replace -1 with FREERTOS_INVALID_SOCKET.
    return (BIP_Socket != FREERTOS_INVALID_SOCKET);
}
...
{       /* number of bytes of data */
    ...
    // FreeRTOS Modification - Replace < 0 with == FREERTOS_INVALID_SOCKET.
    if (BIP_Socket == FREERTOS_INVALID_SOCKET) {
        // FreeRTOS Modification - Replace BIP_Socket with -1.
        return -1;
    }
    ...
    /* Send the packet */
    // FreeRTOS Modification - Use FreeRTOS_sendto.
    struct freertos_sockaddr temp = {
        .sin_addr = bip_dest.sin_addr.s_addr,
        .sin_port = bip_dest.sin_port
    };
    bytes_sent =
        FreeRTOS_sendto(BIP_Socket, (char *) mtu, mtu_len, 0,
        &temp, sizeof(temp));
    ...
    unsigned timeout)
{
    ...
    // FreeRTOS Modification - Remove three lines.
    /*fd_set read_fds;
    int max = 0;
    struct timeval select_timeout;*/
    struct sockaddr_in sin = { 0 };
    // FreeRTOS Modification - Remove line.
    ...
    /* we could just use a non-blocking socket, but that consumes all
       ...
       a select. */
    // FreeRTOS Modification - Don't use select.
    /*if (timeout >= 1000) {
    ...
    uint32_t size = sizeof(temp);
        received_bytes =
            FreeRTOS_recvfrom(BIP_Socket, (char *) &pdu[0], max_pdu, 0,
            &temp, &size);
    /*else
        return 0;*/
